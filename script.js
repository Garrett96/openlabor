        ...
        function renderEntries() {
            ...
            entries.forEach(entry => {
                ...
                if (isEditing) {
                    ...
                    hoursCell = `<span style="font-style: italic; color: #888;">Pending</span>`;
                    costCell = `<span style="font-style: italic; color: #888;">Pending</span>`;

                    otCell = `<input type="checkbox" class="ot-toggle" data-id="${entry.id}" ${entry.isOvertime ? 'checked' : ''}>`;

                    actionsCell = `
                    <button class="save-btn update-btn" data-id="${entry.id}">Update</button>
                    <button class="delete-btn cancel-edit-btn" data-id="${entry.id}">Cancel</button>
                    `;
                }
                else if (isActive) {
                    nameCell = entry.name;
                    ...
                    costCell = '-';
                    otCell = '-';
                    actionsCell = `<button class="save-btn" data-id="${entry.id}">Save</button> <button class="delete-btn" data-id="${entry.id}">Delete</button>`;
                }
                else {
                    ...
                    otCell = `<input type="checkbox" class="ot-toggle" data-id="${entry.id}" ${entry.isOvertime ? 'checked' : ''}>`;
                    actionsCell = `<button class="edit-btn" data-id="${entry.id}">Edit</button> <button class="delete-btn" data-id="${entry.id}">Delete</button>`;
                }
